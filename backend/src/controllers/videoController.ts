...
export const generate = async (req: Request, res: Response) => {
  try {
    ...
    const job = await createVideoJob({ prompt, image, aspectRatio }, String(req.id));
    res.status(202).json({
      jobId: job.id,
      status: job.status,
      requestId: req.id,
...
export const status = async (req: Request, res: Response) => {
  try {
    ...
    logger.error({ err: error, operation: req.body.operation, jobId: req.body.jobId }, 'Failed to check video status');
    res.status(500).json({
