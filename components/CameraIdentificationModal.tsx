...
const CameraIdentificationModal: React.FC<CameraIdentificationModalProps> = ({
  onClose,
  onIdentify,
}) => {
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      setError(
        'No se pudo acceder a la cámara. Puedes intentar introducir el número de serie manualmente abajo.'
      );
    }
  }, []);
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      if (!result.model && !result.serialNumber) {
        throw new Error(
          'No se pudo identificar el modelo ni el número de serie. Intenta con una foto más clara y nítida de la etiqueta.'
        );
      }
      stopCamera();
      onIdentify(result);
  ...
  const handleManualSubmit = (e: React.FormEvent) => {
    ...
    if (result) {
      ...
      setError(
        "Número de serie no reconocido. Asegúrate de que es un número de serie válido de Nespresso Profesional (ej: contiene una 'Z' en la 6ª posición para Zenius)."
      );
    }
  };
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-lg shadow-2xl w-full max-w-2xl relative max-h-[90vh] overflow-y-auto flex flex-col"
          ...
          {isLoading ? (
            <div className="text-center py-8">
              ...
              <p className="mt-4 text-gray-600">
                Analizando imagen... Esto puede tardar un momento.
              </p>
            </div>
          ) : (
            <>
              <div className="w-full mb-6">
                <p className="text-center text-gray-600 mb-4 text-sm">
                  Apunta la cámara a la etiqueta del número de serie.
                </p>
                {error && (
                  <p className="bg-red-100 text-red-700 p-3 rounded-md mb-4 w-full text-center text-sm">
                    {error}
                  </p>
                )}
              ...
              <form
                ...
                className="w-full max-w-md bg-gray-50 p-4 rounded-lg border border-gray-200"
              >
                <label
                  htmlFor="manualSerial"
                ...
                <p className="text-xs text-gray-500 mt-2">
                  El asistente detectará el modelo automáticamente basándose en el código del número
                  de serie.
                </p>
              </form>
            </>
