  ...
  return (
    <div className="p-4 my-8 max-w-4xl mx-auto animate-fade-in">
      <h2 className="text-xl text-gray-700 font-bold mb-2 text-center">
        Base de Conocimiento de Fallos Comunes
      </h2>
      <p className="text-gray-500 mb-6 text-center">
        Aquí tienes ayuda para los problemas más habituales. Haz clic en uno para ver la solución o
        busca tu problema.
      </p>
        ...
        {filteredFaults.length === 0 ? (
          <div className="text-center py-8 px-4 text-gray-500 bg-white rounded-lg border border-dashed border-gray-300 shadow-sm">
            <p className="mb-4 text-lg">
              No hemos encontrado problemas guardados que coincidan con &quot;
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              <button
                onClick={() =>
                  onProblemSelect(`Tengo un problema que no está en la lista: ${searchTerm}`, false)
                }
              ...
              <button
                onClick={() =>
                  onProblemSelect(
                    `Busca en la web información reciente sobre cómo solucionar este problema en una cafetera Nespresso profesional: ${searchTerm}`,
                    true
                  )
                }
          ...
          filteredFaults.map((fault) => (
            <div
              key={fault.id}
              className="border border-gray-200 rounded-lg shadow-sm overflow-hidden bg-white transition-shadow hover:shadow-md"
            >
              <button
              ...
              >
                ...
                <ChevronDownIcon
                  className={`w-5 h-5 text-gray-500 transition-transform duration-300 ${openId === fault.id ? 'rotate-180' : ''}`}
                />
              </button>
              {openId === fault.id && (
                <div
                  id={`fault-${fault.id}`}
                  className="p-5 border-t border-gray-200 bg-gray-50/70 animate-fade-in-slow"
                >
                  <div className="mb-6">
                          ...
                          <p className="flex-1 text-gray-800 pt-1">
                            {step.description.split(/(\*\*.*?\*\*)/g).map((part, i) => {
                              if (part.startsWith('**') && part.endsWith('**')) {
                                return (
                                  <strong key={i} className="font-bold text-blue-600">
                                    {part.slice(2, -2)}
                                  </strong>
                                );
                              }
                  ...
                  <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    ...
                    <ul className="list-disc list-inside space-y-1 text-yellow-700">
                      {fault.preventionTips.map((tip, index) => (
                        <li key={index}>{tip}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="mt-6 text-center">
                    <button
                      onClick={() =>
                        onProblemSelect(
                          `Hola, necesito más ayuda con este problema: "${fault.symptom}"`,
                          false
                        )
                      }
