...
const Checklist: React.FC<ChecklistProps> = ({ machineModel, serialNumber, items, onClose }) => {
  ...
  const handleTimeChange = (field: 'startTime' | 'endTime', value: string) => {
    if (field === 'startTime') { setStartTime(value); saveMeta({ startTime: value }); }
    else { setEndTime(value); saveMeta({ endTime: value }); }
  };
  ...
  const handleNotesChange = (e: React.ChangeEvent<HTMLTextAreaElement>, field: 'notes' | 'finalNotes') => {
    const text = e.target.value;
    if (field === 'notes') { setNotes(text); saveMeta({ notes: text }); }
    else { setFinalNotes(text); saveMeta({ finalNotes: text }); }
  };
  ...
  return (
    <div className="p-4 my-4 max-w-4xl mx-auto border border-blue-200 rounded-lg shadow-lg bg-white animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 bg-gray-50 p-4 rounded-md">
        <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Nombre del Técnico</label>
            <input 
                type="text" 
        ...
        <div className="flex gap-4">
             ...
             <div className="flex-1">
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Hora Final</label>
                <input 
                    type="time" 
                    value={endTime} 
        ...
        <div className="flex items-center mt-2">
             <label className="flex items-center cursor-pointer">
                <div className="relative">
                    <input type="checkbox" className="sr-only" checked={extraibles} onChange={handleExtraiblesChange} />
                    <div className={`block w-10 h-6 rounded-full transition-colors ${extraibles ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                    <div className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${extraibles ? 'transform translate-x-full' : ''}`}></div>
                </div>
      ...
      <div className="mb-4 bg-blue-50 p-3 rounded border border-blue-100">
         <label className="block text-xs font-bold text-blue-800 uppercase mb-1">Descripción Avaria / Notas Iniciales</label>
         <textarea
            value={notes}
      ...
      <div className="space-y-6">
        {Object.entries(itemsBySection).map(([section, sectionItems]: [string, ChecklistItem[]]) => (
            <div key={section}>
                <h3 className="text-md font-semibold text-gray-700 border-b border-gray-200 pb-1 mb-2 bg-gray-100 px-2 py-1 rounded-t">{section}</h3>
                    ...
                    <li key={item.id}>
                        <label className="flex items-start p-2 hover:bg-gray-50 rounded cursor-pointer transition-colors border-b border-gray-50 last:border-0">
                            ...
                            <div className="ml-3 text-sm">
                                <span className={`font-medium text-gray-800 ${checkedItems[item.id] ? 'text-green-700' : ''}`}>
                                    {item.text}
                                </span>
                            </div>
                            {checkedItems[item.id] && <span className="ml-auto text-xs font-bold text-green-600 px-2">OK</span>}
                        </label>
                    </li>
                    ))}
                </ul>
